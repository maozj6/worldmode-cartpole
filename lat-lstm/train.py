...
from lat_loader import MDNDataset
import argparse
import torch
import torch.nn as nn
import torch.nn.functional as f
...
if __name__ == '__main__':
    ...
    device="cuda"
    print(device)
    latrnn = latent_lstm(32, 1, 256).to(device)
    parser = argparse.ArgumentParser(description='New latent Trainer')

    parser.add_argument('--train', default="/home/mao/24Spring/Clip/cartpoledata/496_real_large/test/controller1/",
                        help='Best model is not reloaded if specified')
    parser.add_argument('--test',default="/home/mao/24Spring/Clip/cartpoledata/496_real_large/test/controller1/",
    ...
    optimizer = torch.optim.RMSprop(latrnn.parameters(), lr=1e-3, alpha=.9)
    ...
    for epoch in range(600):
        ...
        for i, data in enumerate(train_loader):
            ...
            zoutputs = latrnn(acts.view(BSIZE,32,1), latent_obs)
            gmm = criterion(latent_next_obs, zoutputs)
        ...
        if is_best:
            cur_best = test_loss
        checkpoint_fname =  'reallatlstmcheckpoint'+'.tar'
        save_checkpoint({
            "state_dict": latrnn.state_dict(),

...
from mdrnn import MDRNN, gmm_loss,MDRNNCell
import torch
import torch.nn as nn
...
if __name__ == '__main__':
    ...
    parser.add_argument('-m', '--mdn', type=str,default="mdnbest.tar", help='the path of mdn model')
    ...
    mdrnn = MDRNNCell(32, 1, 256, 5).to(device)
        ...
        for largej in range(0,len(prednumberlist)):
            ...
            for i in range(len(sequenceobs)):
                ...
                for initi in range(len(inputs)):
                    mus, sigmas, logpi, rs, ds, hidden = mdrnn(acts[initi].view(1, 1),
                                                               latent_obs[0][0][initi].view(1, 32), hidden)

                mixt = Categorical(torch.exp(logpi.squeeze())).sample().item()
                latent_state = mus[:, mixt, :]  # + sigma[:, mixt, :] * torch.randn_like(mu[:, mixt, :])

                decoded_obs = vae.decode(latent_state).squeeze(0)
                ...
                for initi in range(len(next_obs)):
                    ...
                    mus, sigmas, logpi, rs, ds, hidden = mdrnn(thisacts,
                                                               latent_state.view(1, 32), hidden)
                    mixt = Categorical(torch.exp(logpi.squeeze())).sample().item()
                    latent_state = mus[:, mixt, :]  # + sigma[:, mixt, :] * torch.randn_like(mu[:, mixt, :])
                    decoded_obs = vae.decode(latent_state)

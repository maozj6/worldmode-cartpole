...
class _RolloutDataset(torch.utils.data.Dataset): # pylint: disable=too-few-public-methods
    def __init__(self, root, leng=8, horizon=10): # pylint: disable=too-many-arguments
        self.leng=leng
    ...
    def __getitem__(self, i):
        ...
        #     i=number
            ...
            if (len(data['observations'])-self.leng-self.horizon<1):
                i=np.random.randint(0, self._cum_size[-1])
                continue
            if seq_index >= (len(data['observations'])-self.leng-self.horizon):
                seq_index = np.random.randint(0,len(data['observations'])-self.leng-self.horizon)
            break
...
class MDNDataset(_RolloutDataset): # pylint: disable=too-few-public-methods
    ...
    def _get_data(self, data, seq_index):
        img = data['observations'][seq_index:seq_index+33]
        action = data['actions'][seq_index:seq_index+32]
...
class MDNDataset(_RolloutDataset): # pylint: disable=too-few-public-methods
    ...
    def _get_data(self, data, seq_index):
        img = data['observations'][seq_index:seq_index+self.leng+self.horizon]
        safes = data['safes'][seq_index:seq_index+self.leng+self.horizon]

        action = data['actions'][seq_index:seq_index+self.leng]
        # new_image = Image.fromarray(img)
        # grayscale_img = new_image.convert('L')
        # img = np.array(grayscale_img)
        obs = torch.tensor(img)
        ...
        return tensor_img[0:self.leng],tensor_img[self.leng:self.leng+self.horizon],action,safes
